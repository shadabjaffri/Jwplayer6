package com.longtailvideo.jwplayer.controller {
	...
	public class Controller extends GlobalEventDispatcher {
		...
		protected function addViewListeners() : void {
         this._view.addEventListener(ViewEvent.JWPLAYER_VIEW_PLAY,this.playHandler);
         this._view.addEventListener(ViewEvent.JWPLAYER_VIEW_PAUSE,this.pauseHandler);
         this._view.addEventListener(ViewEvent.JWPLAYER_VIEW_STOP,this.stopHandler);
         this._view.addEventListener(ViewEvent.JWPLAYER_VIEW_NEXT,this.nextHandler);
         this._view.addEventListener(ViewEvent.JWPLAYER_VIEW_PREV,this.prevHandler);
         this._view.addEventListener(ViewEvent.JWPLAYER_VIEW_SEEK,this.seekHandler);
         this._view.addEventListener(ViewEvent.JWPLAYER_VIEW_MUTE,this.muteHandler);
         this._view.addEventListener(ViewEvent.JWPLAYER_VIEW_VOLUME,this.volumeHandler);
         this._view.addEventListener(ViewEvent.JWPLAYER_VIEW_FULLSCREEN,this.fullscreenHandler);
         this._view.addEventListener(ViewEvent.JWPLAYER_VIEW_LARGEPLAYER,this.largePlayerHandler);
         this._view.addEventListener(ViewEvent.JWPLAYER_VIEW_LOAD,this.loadHandler);
         this._view.addEventListener(ViewEvent.JWPLAYER_VIEW_REDRAW,this.redrawHandler);
      }
		...
		protected function largePlayerHandler(param1:ViewEvent) : void {
         this.largePlayer(param1.data);
      }
...
*/
		...
		public function largePlayer(param1:Boolean) : Boolean {
         if(param1 != this._model.largePlayer)
         {
            this._model.largePlayer = param1;
            this._view.largePlayer(param1);
            dispatchEvent(new PlayerEvent(PlayerEvent.JWPLAYER_LARGEPLAYER,param1.toString()));
            return true;
         }
         return false;
      }
